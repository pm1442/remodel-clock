...
function App() {
  ...
  return <main className="app-shell"><header><div><div className="wordmark">RIDGEPOINT</div><span>Remodel Clock</span></div><div className="user"><strong>{profile.full_name}</strong><button className="text-button" onClick={signOut}>Sign out</button></div></header><nav>{(['jobs', 'clock', 'timesheet'] as const).map((item) => <button key={item} className={tab === item ? 'active' : ''} onClick={() => setTab(item)}>{item === 'jobs' ? 'Jobs' : item === 'clock' ? 'Time clock' : 'Timesheet'}</button>)}</nav>{notice && <div className="notice">{notice}<button onClick={() => setNotice('')}>Dismiss</button></div>}
    {tab === 'jobs' && <section><div className="section-head"><div><p className="kicker">JOB BOARD</p><h1>Pending jobs</h1></div><button onClick={() => setShowJobForm(true)}>Add job</button></div><div className="job-grid">{jobs.length ? jobs.map((job) => <article className="job" key={job.id} onClick={() => { setSelectedJobId(job.id); setTab('clock'); }}><span className={`color ${COLORS[job.color].className}`}></span><div><h2>{job.title}</h2><p>{job.client_name}{job.address ? ` | ${job.address}` : ''}</p><small className={COLORS[job.color].className}>{job.status}</small></div><b>›</b></article>) : <div className="empty">No jobs yet. Add the first pending job.</div>}</div></section>}
    {tab === 'clock' && <section className="clock"><p className="kicker">TIME CLOCK</p><h1>{active ? 'You are clocked in' : 'Clock time'}</h1><div className="clock-status"><small>{active ? 'CLOCKED IN AT' : 'SELECT A JOB'}</small><strong>{active ? formatTime(active.clock_in) : 'Ready when you are'}</strong><span>{active?.jobs?.title ?? currentJob?.title ?? 'Choose a job below.'}</span></div>{!active && <label className="job-select">Working on<select value={selectedJobId} onChange={(e) => setSelectedJobId(e.target.value)}><option value="">Select a job</option>{jobs.map((job) => <option key={job.id} value={job.id}>{job.title} - {job.client_name}</option>)}</select></label>}<button className="wide" onClick={active ? clockOut : clockIn}>{active ? 'Clock out' : 'Clock in'}</button><p className="helper">All time is attached to the selected job. Adjust a missed punch from the timesheet.</p></section>}
    {tab === 'timesheet' && <section><p className="kicker">CURRENT PAY PERIOD</p><h1>{formatDate(start)} - {formatDate(end)}</h1><p className="period-note">Two weeks, Thursday through Wednesday</p><div className="hours"><strong>{hours.toFixed(2)}</strong><span>Total hours</span></div><div className="entries">{periodEntries.length ? periodEntries.map((entry) => <article className="entry" key={entry.id}><div><h2>{entry.jobs?.title ?? 'Job'}</h2><p>{formatDate(new Date(entry.clock_in))} | {formatTime(entry.clock_in)}{entry.clock_out ? ` - ${formatTime(entry.clock_out)}` : ' - still clocked in'}</p></div><div><strong>{toHours(entry).toFixed(2)} h</strong>{entry.clock_out && <button className="text-button" onClick={() => adjust(entry)}>Adjust</button>}</div></article>) : <div className="empty">No time entered in this pay period.</div>}</div><button className="wide ghost" onClick={() => window.print()}>Print timesheet</button></section>}
